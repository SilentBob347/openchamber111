...
- Every unit, contract and escape test and the live tests of the server inside: 774 tests in 131 seconds. The escape suite runs 56 test cases, from 41 `it` blocks and 3 `it.each` tables, 22 of them about the gatekeeper. The contract suite runs 13 and has the three added assertions and no changed one.
- A create with the gatekeeper costs 2.9 seconds when the tools volume is filled, against the 2.3 seconds stage 1b measured without one, and 38.9 seconds when it pays for the fill. `stop` takes 0.2 seconds and `start` 2.5 seconds. The live contract file, one fill plus the whole scenario, took 43 to 49 seconds, and the whole live run of the module 131 seconds.
...
- The socket flood opens 400 connections to each of the three listeners from inside the space and holds them. It passes only when the gatekeeper is alive afterwards, its journal is still readable and holds at most a few notes about the flood, the corridor serves again, and the space still verifies clean.
- The unit tests flood the program on the host, and they run it with small caps for it: 20, 4 and 4 in place of 128, 64 and 8, with the flood a small multiple of each. That is not a convenience. A drop happens when a connection is *accepted* past the cap, and a machine with no spare capacity accepts slowly while the kernel's backlog keeps completing handshakes, so 300 clients against a cap of 128 can all report themselves connected with nothing accepted past the cap and nothing to record. It passed on three machines and failed in CI, which runs 245 test files at once. The corridor's test cap stays above the 16 refused clients the program waits on at once, or a refusal would fill it, which is why that one is 20 and not 4. Every note the floods assert is waited for, like every other record these tests read.

  Small caps are also what keeps this file affordable on a Windows host, where a closed port waits two minutes in TIME_WAIT before it comes back. The two loops that dominate it cost 124 sockets now instead of 240: 60 for the refusals that have to outnumber the cap, and 64 across the three floods.

- **On Windows, with the live Docker tests enabled, run the spaces folder one file at a time.** Measured on Windows 11 with Docker Desktop 29.6.2, on the revision before the caps came down: the file alone, `--no-file-parallelism`, 206 passed; the whole folder with `OPENCHAMBER_TEST_DOCKER=1` at default parallelism, the worker running `gatekeeper-program.test.js` died with "Worker exited unexpectedly" and 559 of 778 reported; the same folder with `--no-file-parallelism`, 761 passed and 17 skipped of 778 with no leftovers, the 13 difference from the other hosts being the `tools-filler` group that stage 1b switches off there. The file dies only beside the three live Docker files, which hold the daemon for five to six minutes each while this one opens sockets in hundreds. The caps above were halved to make it cheap everywhere rather than special on one platform, and whether that is enough for a parallel run there is not yet known: nothing claims it until a Windows run says so.
